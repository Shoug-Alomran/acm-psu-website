...
async function start(): Promise<void> {
  ...
  async function openApplication(application: ApplicationRow): Promise<void> {
    const notes = await applicationNotes(application.id);
    const decided = application.status === 'approved' || application.status === 'rejected';

    const noteField = field({ label: 'Add an interview note', name: 'note',
      type: 'textarea', rows: 3,
      hint: 'Internal only. Applicants can never read these.' });

    const decisionForm = h('form', { class: 'portal-form' },
      h('div', { class: 'field-pair' },
        field({ label: 'Position on approval', name: 'position_id', type: 'select',
                value: positionList.find((p: Position) => p.slug === 'member')?.id,
                options: [{ value: '', label: '— no position yet —' },
                  ...positionList.map((p: Position) => ({ value: p.id, label: p.title }))] }),
        field({ label: 'Membership start date', name: 'start_date', type: 'date',
                value: new Date().toISOString().slice(0, 10) })),
      reasonField({
        label: 'Reason for this decision',
        hint: 'Required when declining, and recorded on the permanent audit ' +
              'record either way. Shown to the applicant on their status page.',
      }),
      internalNoteField(),
    ) as HTMLFormElement;

    const modal = dialog(application.full_name,
      h('div', { style: { display: 'flex', flexDirection: 'column', gap: '1.5rem' } },
        metaList([
          ...
          ['Account email', application.applicant?.email ?? '—'],
        ...
        application.goal_text
          ? h('div', {},
              h('p', { class: 'mono-meta dim-text' }, 'WANTS EXPERIENCE IN'),
              h('p', application.goal_text))
          : null,

        historyPanel('application', application.id, application.user_id),

        h('div', {},
          h('p', { class: 'mono-meta dim-text' }, 'INTERVIEW NOTES'),
          notes.length
            ? h('div', { class: 'history-list' },
                notes.map((note) => h('div', { class: 'history-row' },
                  h('span', { class: 'mono-meta' }, archiveDateTime(note.created_at)),
                  h('span', note.body))))
            : h('p', { class: 'mono-meta dim-text' }, 'None yet.')),

        h('form', { class: 'portal-form' }, noteField,
          h('div', { class: 'button-row' },
            action('Add note', async () => {
              const body = (noteField.querySelector('textarea') as HTMLTextAreaElement).value.trim();
              if (!body) return;
              await addApplicationNote(application.id, body);
              modal.close();
              toast('Note saved.');
              await draw();
            }))),

        decided ? notice('info', 'This application has already been decided.') : decisionForm,
      ),
      ...
      decided ? null : h('div', { class: 'button-row' },
        ...
        application.status === 'submitted'
          ? action('Mark for interview', async () => {
              await markForInterview(application.id,
                textOf(formValues(decisionForm), 'reason') || null);
              modal.close();
        ...
        action('Not accepted', async () => {
          const values = formValues(decisionForm);
          // Declining without an explanation tells a future committee nothing,
          // and leaves the applicant with no idea what happened.
  ...
  async function draw(): Promise<void> {
    const rows = await applications(FILTERS[filter]![1]);
    ...
    render(content,
      ...
      panel(`${rows.length} application${rows.length === 1 ? '' : 's'}`,
        rows.length
          ? dataTable(
              ['Applicant', 'Student ID', 'Major', 'Year', 'Interests', 'Submitted', 'Status', ''],
              rows.map((application) => [
                h('div', {},
                  h('strong', application.full_name),
                  h('p', { class: 'mono-meta dim-text' }, application.psu_email)),
                h('span', { class: 'mono-meta' }, application.student_id),
                application.major,
                h('span', { class: 'mono-meta' }, application.academic_year),
                ...
                h('button', { type: 'button', class: 'link-button',
                  onclick: () => void openApplication(application) }, 'REVIEW'),
              ]))
          : emptyState('Nothing in this queue.',
