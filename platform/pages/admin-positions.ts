...
async function holders(): Promise<Holder[]> {
  const {
    ...
    .select(`
      ...
      ended_on,
      position_id,
      member:app_users!position_history_user_id_fkey(
        full_name
      )
    `)
    .is(
    ...
    const form = h(
      ...
      h(
        ...
        field({
          ...
          hint:
            'Lower sorts first. Executive roles use small numbers.',
        }),
      ),
          ...
          action(
            ...
            async () => {
              ...
              try {
                ...
                if (existing) {
                  /*
                   * The positions table carries an audit trigger, so the
                   * change and its record are one transaction.
                   */
                  const {
                    error,
                  } = await client
                ...
                toast(
                  existing
                    ? `Could not update position: ${errorMessage(error)}`
                    : `Could not create position: ${errorMessage(error)}`,
                  'err',
                );
              }
            },
  ...
  async function draw(): Promise<void> {
    ...
    try {
                      ...
                      action(
                        ...
                        async () => {
                          ...
                          error
                            ...
                            console.error(
                              'Could not change position state:',
                              error,
                            );
      ...
      render(
        ...
        notice(
          'err',
          `The position catalogue could not load: ${errorMessage(error)}`,
        ),
